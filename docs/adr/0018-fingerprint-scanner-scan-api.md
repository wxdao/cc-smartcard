...
`scan()` returns a single player identity table, `{ uuid = playerUuid, name = playerName }`, for the next scan after the call begins. Lua programs should call it as `local scan, err = scanner.scan()`. It has no default timeout and the scanner has no persistent "scanning" state, so Lua programs may safely abandon a call with `parallel.waitForAny`. If the scanner is destroyed or the attached computer stops before a scan arrives, the pending call returns `nil, "scan cancelled"` instead of returning a fake identity.
